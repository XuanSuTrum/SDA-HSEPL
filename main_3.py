...
def segmented_function(epoch):
    if epoch <= 10:
        ...
        value = 2/ (1 + math.exp(-10 * (args.n_epoch) / args.n_epoch)) - 1.7
    elif 40 < epoch <= 85:
        # 在10-30之间逐渐减小值，你可以根据需要调整
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer):
    ...
    for e in range(args.n_epoch):
        ...
        for mlen in range(n_batch):
            ...
            optimizer.zero_grad()
            label_source_pred, transfer_loss, cmmd_loss, sim_matrix, estimated_sim_truth, sim_matrix_target, estimated_sim_truth_target = model(e,data_source, data_target,label_source)
            ...
            loss = clf_loss + beta*transfer_loss+ cmmd_weight*cmmd_loss + cmmd_weight*cls_loss + 2*(e/args.n_epoch)*cluster_loss
        ...
        # print('Epoch: [{:2d}/{}], cls_loss: {:.4f}, transfer_loss: {:.4f}, train_loss_cmmd_loss: {:.4f},train_loss_cls: {:.4f},train_loss_cluster: {:.4f},total_Loss: {:.4f}, acc: {:.4f},accs: {:.4f}'.format(
        #          e, args.n_epoch, train_loss_clf.avg, train_loss_transfer.avg, train_loss_cmmd_loss.avg, train_loss_cls.avg,train_loss_cluster.avg,
        #          train_loss_total.avg, acc,accs))
        if best_acc < acc:
            best_acc = acc
