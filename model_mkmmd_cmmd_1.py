...
class Transfer_Net(nn.Module):
    def __init__(self, num_class, base_net='CFE', transfer_loss='mmd', use_bottleneck=False, width=32,
    ...
    def forward(self, e, source, target, s_label):
        ...
        return source_clf, transfer_loss, cmmd_loss, sim_matrix, estimated_sim_truth
    ...
    def visualization(self, source, source_labels, target, target_labels, tsne=1):
        ...
        if tsne == 0:
            ...
            plt.scatter(x0_source[:, 0], x0_source[:, 1], c='none', marker='o', edgecolors=colors1, alpha=0.1,
                        label='Source Class 0')
            plt.scatter(x1_source[:, 0], x1_source[:, 1], c='none', marker='o', edgecolors=colors1, alpha=0.1,
                        label='Source Class 1')
            plt.scatter(x2_source[:, 0], x2_source[:, 1], c='none', marker='o', edgecolors=colors1, alpha=0.1,
                        label='Source Class 2')

            plt.scatter(x0_target[:, 0], x0_target[:, 1], c='none', marker='*', edgecolors=colors2, alpha=0.5,
                        label='Target Class 0')
            plt.scatter(x1_target[:, 0], x1_target[:, 1], c='none', marker='*', edgecolors=colors2, alpha=0.5,
                        label='Target Class 1')
            plt.scatter(x2_target[:, 0], x2_target[:, 1], c='none', marker='*', edgecolors=colors2, alpha=0.5,
                        label='Target Class 2')
