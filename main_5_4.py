...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer):
    ...
    for e in range(args.n_epoch):
        ...
        for mlen in range(n_batch):
            ...
            # 1
            # loss = clf_loss + cmmd_weight * cmmd_loss + cmmd_weight * cls_loss + 2 * (
            #         e / args.n_epoch) * cluster_loss
            # 2
            # loss = clf_loss + beta * transfer_loss + cmmd_weight * cls_loss + 2 * (
            #         e / args.n_epoch) * cluster_loss
            # 3 no source pair
            # loss = clf_loss + beta * transfer_loss + cmmd_weight * cmmd_loss + 2 * (
            #         e / args.n_epoch) * cluster_loss
            # 4 no target pair
            loss = clf_loss + beta * transfer_loss + cmmd_weight * cmmd_loss + cmmd_weight * cls_loss
...
if __name__ == '__main__':
    ...
    # 2. 计算每个数据与均值的差的平方
    squared_differences = [(x - average_result) ** 2 for x in stacked_results]

    # 3. 求平方差的平均值
    variance = sum(squared_differences) / len(stacked_results)
    variance_cpu = variance.cpu()

    # 然后可以使用 numpy 进行计算
    std_dev = np.sqrt(variance_cpu.numpy())
